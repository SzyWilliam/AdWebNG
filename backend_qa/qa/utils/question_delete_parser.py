...
class QuestionDeleteParser:
    ...
    def delete_parse(self, classify_result):
    ...
    def sql_transfer(self, question_type, keywords):
        ...
        if question_type == 'disease_cause':
            sql = ["MATCH (m:Disease) where m.name = '{0}' WITH m, m.cause AS t REMOVE m.cause RETURN t".format(i) for i
                   in keywords]

        # 预防
        elif question_type == 'disease_prevent':
            sql = ["MATCH (m:Disease) where m.name = '{0}' WITH m, m.prevent AS t REMOVE m.prevent RETURN t".format(i)
                   for i in keywords]

        # 持续时间
        elif question_type == 'disease_last_time':
            sql = [
                "MATCH (m:Disease) where m.name = '{0}' WITH m, m.cure_lasttime AS t REMOVE m.cure_lasttime RETURN t".format(
                    i) for i in keywords]

        # 治愈概率
        elif question_type == 'disease_cure_prob':
            sql = [
                "MATCH (m:Disease) where m.name = '{0}' WITH m, m.cured_prob AS t REMOVE m.cured_prob RETURN t".format(
                    i) for i in keywords]

        # 治疗方式
        elif question_type == 'disease_cure_way':
            sql = ["MATCH (m:Disease) where m.name = '{0}' WITH m, m.cure_way AS t REMOVE m.cure_way RETURN t".format(i)
                   for i in keywords]

        # 易发人群
        elif question_type == 'disease_easy_get':
            sql = ["MATCH (m:Disease) where m.name = '{0}' WITH m, m.easy_get AS t REMOVE m.easy_get RETURN t".format(i)
                   for i in keywords]

        # 疾病介绍
        elif question_type == 'disease_desc':
            sql = ["MATCH (m:Disease) where m.name = '{0}' WITH m, m.desc AS t REMOVE m.desc RETURN t".format(i) for i
                   in keywords]
